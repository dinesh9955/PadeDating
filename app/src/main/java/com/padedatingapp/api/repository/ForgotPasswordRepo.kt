...
class ForgotPasswordRepo @Inject constructor(private val padeApi: PadeDatingApi) {
    ...
    suspend fun forgotPassword(requestBody: RequestBody): Resource<Result<UserModel>> {
        return handleException { padeApi.forgotPassword(requestBody)}
    }

    suspend fun resetPassword(token:String,requestBody: RequestBody): Resource<Result<UserModel>> {
